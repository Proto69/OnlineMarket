<x-app-layout>
    ...
    <div class="hidden sm:block py-12 ">
        <div class="max-w-7xl mx-auto min-w-[905px] sm:px-6 lg:px-8 ">
            <div class="overflow-hidden shadow-xl sm:rounded-lg mr-96 mt-[-0.5rem] bg-transparent">
                ...
                <div class="border border-lime-500 mb-1 dark:border-lime-400 dark:bg-gray-800 h-68 rounded-lg grid grid-cols-3 min-w-[470px]">
                    ...
                    <div class="col-span-1 border-l border-lime-500 dark:border-lime-400 rounded-tr-lg rounded-br-lg flex items-center justify-center min-w-40">
                        <form class="max-w-xs mx-auto" action="{{ route('remove-product-from-cart', $product->id) }}" method="POST">
                            @csrf
            ...
            <div class="fixed top-44 right-6 bottom-12 min-w-[300px] items-center mr-16 bg-white border border-lime-500 rounded-lg shadow-md dark:bg-gray-800 dark:border-lime-400 dark:hover:bg-gray-700 sm:rounded-lg pt-3 w-1/4">
                ...
                $scrollClass = $productCount >= 10 ? 'overflow-y-scroll' : '';
                @endphp
                ...
                <div class="flex flex-col items-center justify-end px-4 mb-4">
                    ...
                    <form action="{{ route('complete-order-cart') }}" method="GET">
                        @csrf
    ...
    <!-- small screens -->
    <div class="sm:hidden py-12 ">
        @if (count($products) > 0)
        @foreach ($products as $product)
        <div class="mt-2 mx-3 card-form relative min-w-80 bg-white dark:bg-gray-800 border border-gray-300 p-4 rounded-lg flex flex-col h-full items-center">

            <div class="mb-2 flex-grow">
                <!-- Product Image -->
                <img class="mt-1 mb-2  productImage" src="{{ $product->getImageURL() }}">
                <!-- Product Name -->
                <h5 class="text-xl font-semibold tracking-tight text-gray-900 dark:text-white text-center">{{ $product->name }}</h5>
            </div>

            <div class="mt-auto mb-2 flex items-center justify-between">
                <form class="max-w-xs mx-auto" action="{{ route('remove-product-from-cart', $product->id) }}" method="POST">
                    @csrf
                    <label for="quantity-input" class="block mb-2 pl-7 text-sm font-medium items-center justify-center text-gray-900 dark:text-white ms-1.5">
                        <strong>Количество</strong></label>
                    <x-input name="product_id" id="default-search" class="hidden" value="{{ $product->id }}" />
                    <div class="relative flex items-center max-w-[9rem]">
                        <button type="button" id="decrement-button-{{ $product->id }}" data-product-id="{{ $product->id }}" data-input-counter-decrement="quantity-input-{{ $product->id }}-sm" class="decrement-button bg-gray-100 dark:bg-gray-700 dark:hover:bg-gray-600 dark:border-gray-600 hover:bg-gray-200 border border-gray-300 rounded-s-lg p-3 h-11 focus:ring-gray-100 dark:focus:ring-gray-700 focus:ring-2 focus:outline-none">
                            <svg class="w-3 h-3 text-gray-900 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 18 2">
                                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M1 1h16" />
                            </svg>
                        </button>
                        <input type="text" id="quantity-input-{{ $product->id }}-sm" data-product-id="{{ $product->id }}" data-input-counter data-input-counter-min="1" data-input-counter-max="{{ $product->quantity }}" aria-describedby="helper-text-explanation" class="quantity-input bg-gray-50 border-x-0 border-gray-300 h-11 text-center text-gray-900 text-sm focus:ring-blue-500 focus:border-blue-500 block w-full py-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" value="{{ $product->bought_quantity }}" required>
                        <button type="button" id="increment-button-{{ $product->id }}" data-product-id="{{ $product->id }}" data-input-counter-increment="quantity-input-{{ $product->id }}-sm" class="increment-button bg-gray-100 dark:bg-gray-700 dark:hover:bg-gray-600 dark:border-gray-600 hover:bg-gray-200 border border-gray-300 rounded-e-lg p-3 h-11 focus:ring-gray-100 dark:focus:ring-gray-700 focus:ring-2 focus:outline-none">
                            <svg class="w-3 h-3 text-gray-900 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 18 18">
                                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 1v16M1 9h16" />
                            </svg>
                        </button>
        ...
        <div class="min-w-[300px] mt-3 items-center bg-white border border-lime-500 rounded-lg shadow-md dark:bg-gray-800 dark:border-lime-400 dark:hover:bg-gray-700 sm:rounded-lg pt-3 mx-3">
            <div class="px-4">
                <h2 class="dark:text-white text-black font-bold px-20 text-xl pb-3">
                    Касова бележка
                </h2>
            ...
            <div class="mx-0 max-h-80 border-y border-lime-500 dark:border-lime-400 py-3">
                <div class="receipt-section">
                    @foreach ($products as $product)
                    <p class="ps-3 dark:text-gray-400 text-gray-900">
                        <strong>{{ $product->name }}</strong> x <strong>{{ $product->bought_quantity }}</strong> = <strong>{{ $product->price * $product->bought_quantity}}</strong>
