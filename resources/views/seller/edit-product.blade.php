        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg">
                <div class="card-form bg-white dark:bg-gray-800 border border-gray-300 p-4 rounded-lg">
                    <form id="newProductForm" enctype="multipart/form-data" action="{{ route('edit-product-save') }}"
                        method="POST">
                        @csrf
                        <!-- Име -->
                        <div class="flex items-center">
                            <h2 class="text-xxl font-semibold text-gray-900 dark:text-white">
                                <label for="quantity-input"
                                    class="block mb-1 text-xl font-medium text-gray-900 dark:text-white">Име:</label>
                                @error('name')
                                    <p class="text-red-500 text-sm mt-1 mb-1">{{ $message }}</p>
                                @enderror
                                <x-input type="text" name="name" class="name"
                        ...
                        <!-- Описание -->
                        <label for="quantity-input"
                            class="block mt-3 text-xl font-medium text-gray-900 dark:text-white">Описание:</label>
                        @error('description')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                        <textarea name="description"
                            class="ps-2 pt-1 pe-2 description border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm mt-1 mb-3"
                            cols="20" rows="5">{{ $product->description }}</textarea>
                        ...
                        <x-input type="text" name="quantity" id="quantity" class="h-11 text-center text-sm py-2.5"
                            value="{{ $product->quantity }}" required></x-input>
                        ...
                        @error('price')
                            <p class="text-red-500 text-sm mt-1 mb-1">{{ $message }}</p>
                        @enderror
                        <x-input name="price" class="ps-2 mb-4 text-center" type="text"
                            value="{{ $product->price }}"></x-input> <br />
